...
flowchart TB
  subgraph SWARM[Docker Swarm]
    subgraph SERVICE[github_docker_runner]
      RUNNER[Container do Runner<br>- Sempre ligado <br>- Conecta ao Docker do host<br>- Registra na organização GitHub]
    end
  end
```
...
flowchart TB
  subgraph K8S[Kubernetes Cluster]
    subgraph SYS[Namespace: arc-systems]
      CONTROLLER[ARC Controller<br>sempre rodando]
      LISTENER[Listener<br>sempre rodando]
    end

    subgraph RUNNERS[Namespace: arc-runners]
      JOBS[Quando há um job no GitHub]
      subgraph POD[Pod do Runner efêmero]
        R[Runner]
        D[DinD<br>Docker]
      end
      JOBS --> POD

      EMPTY[Quando não há jobs: vazio]
    end
  end
```
